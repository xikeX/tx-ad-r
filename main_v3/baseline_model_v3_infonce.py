...
class BaselineModel(torch.nn.Module):
    ...
    def loss(self, log_feats, pos_embs, neg_embs, loss_mask):
        batch_size = pos_embs.shape[0]
        max_len = pos_embs.shape[1]
        neg_embs_number = neg_embs.shape[1] // max_len

        # Reshape neg_embs: [B, K*L, D] -> [B, K, L, D] -> [B, L, K, D]
        neg_embs = neg_embs.unsqueeze(1).reshape(batch_size, neg_embs_number, max_len, -1)
        neg_embs = neg_embs.transpose(1, 2)  # [B, L, K, D]

        # Compute logits
        pos_logits = (log_feats * pos_embs).sum(dim=-1)  # [B, L]
        neg_logits = (log_feats.unsqueeze(1) * neg_embs).sum(dim=-1)  # [B, L, K] 或 [B, K, L]

        # Apply mask
        pos_logits = pos_logits * loss_mask
        neg_logits = neg_logits * loss_mask.unsqueeze(-1)  # 注意维度对齐

        # Flatten using mask
        valid_mask = loss_mask.bool()  # [B, L]

        log_feats = log_feats[valid_mask]  # [num_valid, D]
        pos_embs = pos_embs[valid_mask]    # [num_valid, D]
        neg_embs = neg_embs[valid_mask]    # [num_valid, K, D]
        anchor_mask = torch.arange(batch_size, device=self.dev).unsqueeze(1).expand(-1, max_len)[valid_mask]  # [num_valid]

        # Compute loss
        loss, pos_sim, neg_sim = info_nce_loss(log_feats, pos_embs, anchor_mask, neg_embs)
