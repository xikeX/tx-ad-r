...
class BaselineModel(torch.nn.Module):
    ...
    def special_embedding_apply(self):
        ...
        self.item_emb.weight.data = torch.arange(self.item_num+1).unsqueeze(1).expand(-1, 32).float().to(self.dev)
        self.user_emb.weight.data = torch.arange(self.user_num+1).unsqueeze(1).expand(-1, 32).float().to(self.dev)
        for k in self.sparse_emb:
            self.sparse_emb[k].weight.data = torch.arange(self.sparse_emb[k].num_embeddings).unsqueeze(1).expand(-1, 32).float().to(self.dev)
        for k in self.ITEM_EMB_FEAT:
            self.emb_transform[k].weight.data = torch.eye(self.ITEM_EMB_FEAT[k]).to(self.dev)
    ...
    def save_item_emb(self, item_ids, retrieval_ids, feat_dict, save_path, batch_size=1024):
        ...
        for start_idx in tqdm(range(0, len(item_ids), batch_size), desc="Saving item embeddings"):
            ...
            # pickle.dump(batch_emb, open("data.pkl",'wb'))
