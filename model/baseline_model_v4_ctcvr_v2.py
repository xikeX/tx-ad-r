...
class BaselineModel(torch.nn.Module):
    ...
    def forward(
        ...
        click_embeds = log_feats + self.click_dnn_layernorm(self.click_dnn(click_feature))
        mask = (next_action_type == 1)
        click_pos_embeds,mask = batch_propagate_last_key_value(mask,pos_embs)
        indices = torch.where(mask)
        pos_logits = (click_embeds * click_pos_embeds).sum(dim=-1) *mask
        neg_logits = (click_embeds * neg_embs).sum(dim=-1) *mask
        click_loss, click_pos_sim, click_neg_sim = self.loss(pos_logits, neg_logits, indices)
    ...
    def loss(self, pos_logits, neg_logits, indices):
