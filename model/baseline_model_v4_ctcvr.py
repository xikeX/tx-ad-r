...
class BaselineModel(torch.nn.Module):
    ...
    def log2feats(self, seq_feature, user_feature, mask):
        ...
        user_bias = self.user_bias_dnn(all_user_embs)
        return log_feats, user_bias
    ...
    def forward(
        ...
        log_feats, user_bias = self.log2feats(seq_feature,user_feature,mask)
        ...
        log_feats = log_feats+user_bias.unsqueeze(1)
        next_action_type_mask = (next_action_type==1)
        pos_logits = (log_feats * pos_embs).sum(dim=-1) *next_action_type_mask
        neg_logits = (log_feats * neg_embs).sum(dim=-1) *next_action_type_mask
        indices = torch.where(next_action_type_mask)
        click_loss, click_pos_sim, click_neg_sim = self.loss(pos_logits, neg_logits, next_action_type)
    ...
    def predict(self, seq_feature, user_feat, mask):
        ...
        final_feat = log_feats[:, -1, :] + user_bias
