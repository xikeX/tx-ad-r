...
class SEBlock(nn.Module):  
    def __init__(self, num_channels, reduction_ratio=4):  
        ...
        # self.avg_pool = nn.AdaptiveAvgPool1d(2)  # 对于1D特征（如嵌入向量）  
    ...
    def forward(self, x_input):  
        b, s, f, size = x_input.size()
        y = F.adaptive_avg_pool1d(x_input.view(-1, size).unsqueeze(1), 1)
        y = y.squeeze().view(b, s, f)
        y = y.view(b, s, f)  
        gate = self.fc(y)
        gate = F.softmax(gate, dim=-1)
        gate = gate.view(b, s, f, 1)  
        return x_input * gate
...
class BaselineModel(torch.nn.Module):
    ...
    def __init__(self, user_num, item_num, feat_statistics, feat_types, args):  #
        ...
        self.senet = SEBlock(self.senet_input_len)
    ...
    def feat2emb(self,
        ...
        batch_size,seq_len = item_sparse_feature.shape[0],item_sparse_feature.shape[1]
        item_sparse_embs = self.item_sparse_emb(item_sparse_feature)# [batch_size, seq_len, sparse_num*hidden_units]
        # 拼接过dnn输出
        item_mm_embs = self.emb_transform(item_mm_embs)
        all_item_emb = torch.cat([item_sparse_embs, item_mm_embs.unsqueeze(-2)], dim=-2)
        all_item_emb = self.senet(all_item_emb)
        all_item_emb = all_item_emb.reshape(batch_size,seq_len,-1)
        ...
        # if include_user:
        #     user_sparse_embs = self.sparse_emb(user_sparse_feature)
        ...
        #     # pre-allocate tensor
        #     batch_emb_data = np.zeros(
        #         (batch_size, seq_len, emb_dim), dtype=np.float32)
