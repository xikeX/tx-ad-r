...
class TrainDataset(torch.utils.data.Dataset):
    ...
    def __init__(self, base_dataset, sample_index=[], neg_sample_size = 10, max_padding_size=100):
        ...
        self.hard_sample = np.random.randint(1, self.base_dataset.itemnum , size=(self.base_dataset.itemnum+1) *10, dtype=np.uint16)
        self.tp = [0]*(self.base_dataset.itemnum+1)
       
    def add_hard_label(self,model):
        # 构建100000的索引，100000的query
        ran = np.random.randint(1, self.base_dataset.itemnum+1 ,size=self.base_dataset.itemnum)
        
        search_size = self.base_dataset.itemnum //10
        query_size = self.base_dataset.itemnum //10//10
        max_size = search_size
        dataset_id = np.zeros(search_size, dtype=np.int64)
        feature = []
        start_time = round(time.time())
        for cnt,item_id in enumerate(ran[:search_size],start=1): 
            dataset_id[cnt-1] = item_id
            feature.append(self.base_dataset.fill_missing_feat(self.base_dataset.item_feat_dict[str(item_id)], item_id))
            if cnt%1000==0:
                end_time = round(time.time())
                use_time = format_time(end_time - start_time)
                remain_time = format_time((end_time - start_time)/(cnt) * max_size)
                print(f"Search feature [{use_time}/{remain_time}] [{cnt}/{max_size}]")
        query_feature = []
        max_size = query_size
        start_time = round(time.time())
        orginal_id = np.zeros(query_size, dtype=np.int64)
        for cnt,item_id in enumerate(ran[search_size:search_size+query_size],start=1): 
            orginal_id[cnt-1] = item_id
            query_feature.append(self.base_dataset.fill_missing_feat(self.base_dataset.item_feat_dict[str(item_id)], item_id))
            if cnt%1000==0:
                end_time = round(time.time())
                use_time = format_time(end_time - start_time)
                remain_time = format_time((end_time - start_time)/(cnt) * max_size)
                print(f" Query feature[{use_time}/{remain_time}] [{cnt}/{max_size}]")

        dataset_vector, final_ids = model.save_item_emb(dataset_id, dataset_id, feature, "", batch_size=1024)
        query_vector, final_ids = model.save_item_emb(orginal_id, orginal_id, query_feature, "", batch_size=1024)
        self.create_faiss(dataset_vector,dataset_id,query_vector,orginal_id)
    def create_faiss(self,dataset_vector,dataset_id,query_vector,orginal_id,faiss_metric_type=1):
        # 在第一轮应该有一定的模型表征能力
        assert len(dataset_vector) == len(dataset_id), "Error: 向量数量和 ID 数量不匹配！"
        ...
        # ================== 2. 设置距离度量 ==================
        space = 'ip' if faiss_metric_type == 1 else 'l2'
        ...
        # 初始化索引结构
        index.init_index(
            max_elements=len(dataset_vector),
            M=faiss_M,
            ef_construction=faiss_ef_construction,
        )
        # 设置查询时的 ef
        index.set_ef(query_ef_search)
        # 可选：启用多线程（添加数据时）
        # index.add_threads = 4  # 可根据 CPU 核心数调整

        # ================== 4. 添加向量和 ID ==================
        print(f"开始构建 HNSW 索引，共 {len(dataset_vector)} 个向量...")
        index.add_items(dataset_vector, dataset_id)
        print("索引构建完成。")
        labels, distances = index.knn_query(query_vector, k=query_ann_top_k)
        # labels: [Q, K] 原始 ID 矩阵
        # distances: [Q, K] 相似度矩阵
        # 将后5个座位难样本
        # 对于每一个向量做两件事情
        start_time = time.time()
        max_size = len(orginal_id)
        for cnt,a_id in enumerate(orginal_id,start=1):
            a_id = int(a_id)
            for retrive_id in labels:
                for b_id in retrive_id[5:]:
                    b_id = int(b_id)
                    if a_id==b_id:
                        continue
                    self.hard_sample[a_id*10+self.tp[a_id]] = b_id
                    self.tp[a_id] = (self.tp[a_id]+1)%10
                    
                    self.hard_sample[b_id*10+self.tp[b_id]] = a_id
                    self.tp[b_id] = (self.tp[b_id]+1)%10
            # 计算时间
            if cnt%1000==0:
                end_time = round(time.time())
                use_time = format_time(end_time - start_time)
                remain_time = format_time((end_time - start_time)/(cnt) * max_size)
                print(f"[{use_time}/{remain_time}] [{cnt}/{max_size}]")    
    ...
    def _random_neq(self, item_id):
        # 从难样本中随机取
        pos = random.randint(0,9)
        return self.hard_sample[item_id*10+pos]
            ...
            if next_type == 1 and next_i != 0:
                ...
                for j in range( self.neg_sample_size):
                    if random.random()<0.5:
                        neg_id = self.base_dataset._random_neq(1, self.base_dataset.itemnum + 1, ts) # 随机采集负样本
                    else:
                        neg_id = self._random_neq(i)
