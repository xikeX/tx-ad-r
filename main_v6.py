...
# 自定义模块
from my_dataset_v1_aug_hard import BaseDataset, TrainDataset, EmbeddingDataset, ValidDataset
...
from baseline_model_v6_infonce_hard import BaselineModel as DownstreamModel
from transformers import get_cosine_schedule_with_warmup
import torch
import torch.nn as nn
from pathlib import Path
...
def get_args():
    ...
    parser.add_argument('--train_name',type=str, default="v6", help='训练名称')
...
def train_downstream_model(model, train_dataset:TrainDataset, valid_loader, args, writer, test_dataset = None, test_dataset_2=None):
    global_step = 0
    ...
    if args.use_lr_scheduler_in_downstream:
        total_steps = args.num_epochs * len(train_dataset)
    ...
    for epoch in range(1, args.num_epochs + 1):
        ...
        if epoch!=1:
            ...
            # 生成一些样本
            # train_dataset刷新样本
            train_dataset.add_hard_label(model)
...
def main():
    ...
    # 开始训练
    train_downstream_model(downstream_model, train_dataset, valid_loader, args, writer,test_dataset, test_dataset_2)
