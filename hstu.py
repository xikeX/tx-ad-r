...
class PointwiseAggregatedAttention(nn.Module):
    def __init__(self, d_model, num_heads, relative_attention_num_buckets=32, relative_attention_max_distance=128):
        super().__init__()
...
class HSTUBlock(nn.Module):
    def __init__(self, d_model, num_heads, dropout=0.1,relative_attention_num_buckets=32, relative_attention_max_distance=128):
        super().__init__()
        self.pre_norm = nn.LayerNorm(d_model)
        self.f1 = nn.Linear(d_model, d_model * 4)  # Transform and split
        self.pointwise_attn = PointwiseAggregatedAttention(d_model, num_heads, relative_attention_num_buckets, relative_attention_max_distance)
    ...
    def forward(self, x,attention_mask):
        ...
        # Spatial Aggregation
        av = self.pointwise_attn(v, k, q,attention_mask)
...
class GenRec(nn.Module):
    def __init__(self, d_model, num_heads, num_layers, dropout=0.1,relative_attention_num_buckets=32, relative_attention_max_distance=128):
        super().__init__()
        self.layers = nn.ModuleList([HSTUBlock(d_model, num_heads, dropout,relative_attention_num_buckets, relative_attention_max_distance) for _ in range(num_layers)])

    def forward(self, x,attention_mask):
        for layer in self.layers:
            x = layer(x,attention_mask) + x
        return x
