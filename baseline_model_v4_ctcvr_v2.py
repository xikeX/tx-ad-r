...
def batch_propagate_last_key_value(mask, values):
    """
    mask: [B, N]  bool 或 0/1 tensor
    values: [B, N] tensor 或 list of lists
    输出: [B, N] 每个位置填充为“前面最近一个 mask==1 位置的值”
    """
    B, N = mask.shape
    
    # 创建位置索引 [B, N]
    indices = torch.arange(N, device=mask.device).unsqueeze(0).expand(B, -1)  # [B, N]
    
    # 将 mask 为 False 的位置设为极小值，True 保留索引
    stamps = torch.where(mask.bool(), indices, N+1)  # [B, N]

    # 沿序列维度 cummax，得到“最近一个关键位置的索引”
    latest_indices = torch.flip(torch.cummin(torch.flip(stamps, [-1]), dim=-1)[0], [-1]) # [B, N]
    
    # 构造 batch 维度的索引
    batch_indices = torch.arange(B, device=mask.device).unsqueeze(1).expand(B, N)  # [B, N]
    mask = mask&(stamps!=N+1)
    latest_indices = torch.where(latest_indices!=N+1, latest_indices, 0)
    # 使用高级索引取值
    if isinstance(values, torch.Tensor):
        # values: [B, N]
        result = values[batch_indices, latest_indices]  # [B, N]
    elif isinstance(values, list):
        # values: list of lists
        result = [
            [values[i][latest_indices[i, j].item()] for j in range(N)]
            for i in range(B)
        ]
    else:
        raise TypeError("values must be torch.Tensor or list of lists")
...
class BaselineModel(torch.nn.Module):
    ...
    def __init__(self, user_num, item_num, feat_statistics, feat_types, args):  #
        ...
        self.train_record = ['total_loss', 'explore_loss', 'click_loss', 'click_pos_sim', 'click_neg_sim','explore_pos_sim', 'explore_neg_sim']
        self.eval_record = ['total_loss', 'explore_loss', 'click_loss', 'click_pos_sim', 'click_neg_sim','explore_pos_sim', 'explore_neg_sim']
        ...
        self.user_bias_dnn = torch.nn.Linear(args.hidden_units, args.hidden_units)
        self.user_last_layernorm = torch.nn.LayerNorm(args.hidden_units, eps=1e-8)
        self.click_dnn = torch.nn.Linear(2 * args.hidden_units, args.hidden_units)
        self.click_dnn_layernorm = torch.nn.LayerNorm(args.hidden_units, eps=1e-8)
    ...
    def feat2emb(self,
        ...
        if include_user:
            ...
            return all_embedding, all_user_embs
    ...
    def log2feats(self, seq_feature, user_feature, mask):
        ...
        maxlen = mask.shape[1]
        seqs,all_user_embs = self.feat2emb(**seq_feature,
                             **user_feature,
        ...
        all_user_embs = self.user_last_layernorm(self.user_bias_dnn(all_user_embs))
        return log_feats, all_user_embs
    ...
    def forward(
        ...
        log_feats, all_user_embs = self.log2feats(seq_feature,user_feature,mask)
        ...
        indices = torch.where(next_mask == 1)
        explore_loss, explore_pos_sim, explore_neg_sim = self.loss(pos_logits, neg_logits, indices)
        
        click_feature = torch.cat([log_feats, all_user_embs.unsqueeze(1).repeat(1,seq_len,1)], dim=-1)
        mask = (next_action_type == 1)
        click_pos_embeds,mask = batch_propagate_last_key_value(mask,pos_embs)
        indices = torch.where(mask)
        click_feature = click_feature[indices]
        click_pos_embeds = click_pos_embeds[indices]
        neg_embs = neg_embs[indices]
        click_embeds = self.click_dnn_layernorm(self.click_dnn(click_feature))
        pos_logits = (click_embeds * click_pos_embeds).sum(dim=-1)
        neg_logits = (click_embeds * neg_embs).sum(dim=-1)
        click_loss, click_pos_sim, click_neg_sim = self.loss(pos_logits, neg_logits)

        return {
            "total_loss": explore_loss + click_loss,
            "explore_loss": explore_loss,
            "click_loss": click_loss,
            "click_pos_sim": click_pos_sim,
            "click_neg_sim": click_neg_sim,
            "explore_pos_sim": explore_pos_sim,
            "explore_neg_sim": explore_neg_sim,
        }
    def loss(self, pos_logits, neg_logits, indices=None):
    ...
    def predict(self, seq_feature, user_feat, mask):
        ...
        """
        log_feats,user_bias = self.log2feats(seq_feature,user_feat,mask=mask)
        log_feats = log_feats[:, -1, :]
        click_embeds = torch.cat([log_feats, user_bias],dim=-1)
        final_feat = log_feats + self.click_dnn_layernorm(self.click_dnn(click_embeds))
        return final_feat
    ...
    def save_item_emb(self, item_ids, retrieval_ids, feat_dict, save_path, batch_size=1024):
        ...
        for start_idx in range(0, max_size, batch_size):
            ...
            batch_feat["item_mm_embs"] = torch.tensor(
                np.stack(batch_feat["item_mm_embs"], axis=0),
                dtype=torch.float32  # ✅ 改为 float32（推荐）
            ...
            if cnt%100==0:
                ...
                print(f"[{use_time}/{remain_time}] [{cnt*batch_size}/{max_size}]")
        # 合并所有批次的结果并保存
